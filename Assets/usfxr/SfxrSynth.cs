...
public class SfxrSynth {
	...
	private SfxrSoundAsset	_params;		// Params instance
	...
	private SfxrSoundAsset	_original;						// Copied properties for mutation base
	...
	public SfxrSoundAsset parameters {
		get { return _params; }
		set { _params = value; }
	}
	...
	public void Play() {
		...
		if (true || _cachingNormal || _cachedWave == null) {
	...
	public void PlayMutated(float __mutationAmount = 0.05f, uint __mutationsNum = 15) {
		...
		if (_cachingMutation != -1) {
			...
			_original = Object.Instantiate(_params);
	...
	public void Stop() {
		...
		if (_original != null) {
			_params = Object.Instantiate(_original);
			_original = null;
		}
	}
	...
	public void CacheMutations(uint __mutationsNum = 15, float __mutationAmount = 0.05f, Action __callback = null, bool __isFromCoroutine = false) {
		...
		if (__callback != null) {
			...
			var original = Object.Instantiate(_params);

			for (uint i = 0; i < _cachedMutationsNum; i++) {
				_params.Mutate(__mutationAmount);
				CacheSound();
				_cachedMutations[i] = _cachedWave;
				_params = Object.Instantiate(original);
			}
	 ...
	 */
	public void Reset(bool __totalReset) {
		// Shorter reference
		SfxrSoundAsset pppppppppppppppppp = _params;

		_period = 100.0f / (pppppppppppppppppp.startFrequency * pppppppppppppppppp.startFrequency + 0.001f);
		_maxPeriod = 100.0f / (pppppppppppppppppp.minFrequency * pppppppppppppppppp.minFrequency + 0.001f);

		_slide = 1.0f - pppppppppppppppppp.slide * pppppppppppppppppp.slide * pppppppppppppppppp.slide * 0.01f;
		_deltaSlide = -pppppppppppppppppp.deltaSlide * pppppppppppppppppp.deltaSlide * pppppppppppppppppp.deltaSlide * 0.000001f;

		if (pppppppppppppppppp.waveType == 0) {
			_squareDuty = 0.5f - pppppppppppppppppp.squareDuty * 0.5f;
			_dutySweep = -pppppppppppppppppp.dutySweep * 0.00005f;
		}

		_changePeriod = Mathf.Max(((1f - pppppppppppppppppp.changeRepeat) + 0.1f) / 1.1f) * 20000f + 32f;
		_changePeriodTime = 0;

		if (pppppppppppppppppp.changeAmount > 0.0) {
			_changeAmount = 1.0f - pppppppppppppppppp.changeAmount * pppppppppppppppppp.changeAmount * 0.9f;
		} else {
			_changeAmount = 1.0f + pppppppppppppppppp.changeAmount * pppppppppppppppppp.changeAmount * 10.0f;
		}

		_changeTime = 0;
		_changeReached = false;

		if (pppppppppppppppppp.changeSpeed == 1.0f) {
			_changeLimit = 0;
		} else {
			_changeLimit = (int)((1f - pppppppppppppppppp.changeSpeed) * (1f - pppppppppppppppppp.changeSpeed) * 20000f + 32f);
		}

		if (pppppppppppppppppp.changeAmount2 > 0f) {
			_changeAmount2 = 1f - pppppppppppppppppp.changeAmount2 * pppppppppppppppppp.changeAmount2 * 0.9f;
		} else {
			_changeAmount2 = 1f + pppppppppppppppppp.changeAmount2 * pppppppppppppppppp.changeAmount2 * 10f;
		}

		_changeTime2 = 0;
		_changeReached2 = false;

		if (pppppppppppppppppp.changeSpeed2 == 1.0f) {
			_changeLimit2 = 0;
		} else {
			_changeLimit2 = (int)((1f - pppppppppppppppppp.changeSpeed2) * (1f - pppppppppppppppppp.changeSpeed2) * 20000f + 32f);
		}

		_changeLimit = (int)(_changeLimit * ((1f - pppppppppppppppppp.changeRepeat + 0.1f) / 1.1f));
		_changeLimit2 = (int)(_changeLimit2 * ((1f - pppppppppppppppppp.changeRepeat + 0.1f) / 1.1f));

		if (__totalReset) {
			_masterVolume = pppppppppppppppppp.masterVolume * pppppppppppppppppp.masterVolume;

			_waveType = (uint)pppppppppppppppppp.waveType;

			if (pppppppppppppppppp.sustainTime < 0.01) pppppppppppppppppp.sustainTime = 0.01f;

			float totalTime = pppppppppppppppppp.attackTime + pppppppppppppppppp.sustainTime + pppppppppppppppppp.decayTime;
			if (totalTime < 0.18f) {
				float multiplier = 0.18f / totalTime;
				pppppppppppppppppp.attackTime *= multiplier;
				pppppppppppppppppp.sustainTime *= multiplier;
				pppppppppppppppppp.decayTime *= multiplier;
			}

			_sustainPunch = pppppppppppppppppp.sustainPunch;

			_phase = 0;

			_overtones = (int)(pppppppppppppppppp.overtones * 10f);
			_overtoneFalloff = pppppppppppppppppp.overtoneFalloff;

			_minFrequency = pppppppppppppppppp.minFrequency;

			_bitcrushFreq = 1f - Mathf.Pow(pppppppppppppppppp.bitCrush, 1f / 3f);
			_bitcrushFreqSweep = -pppppppppppppppppp.bitCrushSweep * 0.000015f;
			_bitcrushPhase = 0;
			_bitcrushLast = 0;

			_compressionFactor = 1f / (1f + 4f * pppppppppppppppppp.compressionAmount);

			_filters = pppppppppppppppppp.lpFilterCutoff != 1.0 || pppppppppppppppppp.hpFilterCutoff != 0.0;

			_lpFilterPos = 0.0f;
			_lpFilterDeltaPos = 0.0f;
			_lpFilterCutoff = pppppppppppppppppp.lpFilterCutoff * pppppppppppppppppp.lpFilterCutoff * pppppppppppppppppp.lpFilterCutoff * 0.1f;
			_lpFilterDeltaCutoff = 1.0f + pppppppppppppppppp.lpFilterCutoffSweep * 0.0001f;
			_lpFilterDamping = 5.0f / (1.0f + pppppppppppppppppp.lpFilterResonance * pppppppppppppppppp.lpFilterResonance * 20.0f) * (0.01f + _lpFilterCutoff);
			if (_lpFilterDamping > 0.8f) _lpFilterDamping = 0.8f;
			_lpFilterDamping = 1.0f - _lpFilterDamping;
			_lpFilterOn = pppppppppppppppppp.lpFilterCutoff != 1.0f;

			_hpFilterPos = 0.0f;
			_hpFilterCutoff = pppppppppppppppppp.hpFilterCutoff * pppppppppppppppppp.hpFilterCutoff * 0.1f;
			_hpFilterDeltaCutoff = 1.0f + pppppppppppppppppp.hpFilterCutoffSweep * 0.0003f;

			_vibratoPhase = 0.0f;
			_vibratoSpeed = pppppppppppppppppp.vibratoSpeed * pppppppppppppppppp.vibratoSpeed * 0.01f;
			_vibratoAmplitude = pppppppppppppppppp.vibratoDepth * 0.5f;

			_envelopeVolume = 0.0f;
			_envelopeStage = 0;
			_envelopeTime = 0;
			_envelopeLength0 = pppppppppppppppppp.attackTime * pppppppppppppppppp.attackTime * 100000.0f;
			_envelopeLength1 = pppppppppppppppppp.sustainTime * pppppppppppppppppp.sustainTime * 100000.0f;
			_envelopeLength2 = pppppppppppppppppp.decayTime * pppppppppppppppppp.decayTime * 100000.0f + 10f;
			...
			_phaser = pppppppppppppppppp.phaserOffset != 0.0f || pppppppppppppppppp.phaserSweep != 0.0f;

			_phaserOffset = pppppppppppppppppp.phaserOffset * pppppppppppppppppp.phaserOffset * 1020.0f;
			if (pppppppppppppppppp.phaserOffset < 0.0f) _phaserOffset = -_phaserOffset;
			_phaserDeltaOffset = pppppppppppppppppp.phaserSweep * pppppppppppppppppp.phaserSweep * pppppppppppppppppp.phaserSweep * 0.2f;
			_phaserPos = 0;
			...
			if (pppppppppppppppppp.repeatSpeed == 0.0) {
				_repeatLimit = 0;
			} else {
				_repeatLimit = (int)((1.0-pppppppppppppppppp.repeatSpeed) * (1.0-pppppppppppppppppp.repeatSpeed) * 20000) + 32;
			}
		}
	}
	 ...
	 */
	public bool SynthWave(float[] __buffer, int __bufferPos, uint __length) {
		_finished = false;
